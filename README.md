...
```lua
-- Define systems
targets.systems["web1"] = { address = "192.168.1.10", user = "deploy" }
targets.systems["web2"] = { address = "192.168.1.11", user = "deploy" }
targets.systems["db"] = { address = "192.168.1.20", user = "deploy" }

-- Group systems
targets.groups["web"] = { members = {"web1", "web2"} }
targets.groups["database"] = { members = {"db"} }

-- Check application requirements
tasks["check_requirements"] = {
    handler = function(system)
        log.info("Checking requirements on " .. system.name)
        local result = system:run_command("docker -v")
        return result.exit_code == 0
    end,
    tags = {"deploy", "check"}
}
...
tasks["install_docker"] = {
    handler = function(system)
        local docker_installed = tasks["check_requirements"].result
        
        if not docker_installed then
            log.info("Installing Docker on " .. system.name)
...
tasks["pull_images"] = {
    handler = function(system)
        log.info("Pulling application images on " .. system.name)
        return system:run_command("docker pull myapp:latest")
    end,
    dependencies = {"install_docker"},
    tags = {"deploy"},
    groups = {"web"} -- Only run on web servers
}

-- Deploy application
tasks["deploy_app"] = {
    handler = function(system)
        log.info("Deploying application on " .. system.name)
        
        -- Create docker-compose.yml
        local compose_template = local:file("templates/docker-compose.yml").content
        local context = {
            app_name = "myapp",
            image = "myapp:latest",
            port = 8080,
        ...
        local compose_content = template.render(compose_template, context)
        system:file("/opt/myapp/docker-compose.yml").content = compose_content
        
        -- Deploy with docker-compose
        return system:run_command("cd /opt/myapp && docker-compose up -d")
    end,
    dependencies = {"pull_images"},
    tags = {"deploy"},
    groups = {"web"}
}
```
